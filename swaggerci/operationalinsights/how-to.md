# How-To
This document describes how to develop for `Az.OperationalInsights`.

## Building `Az.OperationalInsights`
...
## Testing `Az.OperationalInsights`
...
## Packing `Az.OperationalInsights`
...
There are multiple scripts created for performing different actions for developing `Az.OperationalInsights`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.OperationalInsights.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.OperationalInsights.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.OperationalInsights`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.OperationalInsights` into the session.
