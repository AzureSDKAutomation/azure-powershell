# How-To
This document describes how to develop for `Az.SourceControlConfiguration`.

## Building `Az.SourceControlConfiguration`
...
## Testing `Az.SourceControlConfiguration`
...
## Packing `Az.SourceControlConfiguration`
...
There are multiple scripts created for performing different actions for developing `Az.SourceControlConfiguration`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Az.SourceControlConfiguration.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Az.SourceControlConfiguration.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Az.SourceControlConfiguration`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Az.SourceControlConfiguration` into the session.
